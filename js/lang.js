/* ============================================================
   ENTRE2FIT — Bilingual System (EN / ES)
   ============================================================ */

const LANG_KEY = 'entre2fit_lang';

const translations = {

  en: {
    /* ── Navigation ── */
    nav_home:        'Home',
    nav_shop:        'Shop',
    nav_calc:        'Calculator',
    nav_how:         'How to Buy',
    nav_testimonials:'Testimonials',

    /* ── Hero (index) ── */
    hero_label:   'Clinical-Grade Peptide Science',
    hero_title:   'Elevate Your Biology.<br><span>Unlock Peak Performance.</span>',
    hero_sub:     'Entre2Fit delivers rigorously tested, ultra-pure peptide compounds — formulated for athletes, biohackers, and longevity seekers who demand more.',
    hero_cta1:    'Shop Now',
    hero_cta2:    'How to Buy',
    hero_stat1_val:'500+',
    hero_stat1_lbl:'Happy Customers',
    hero_stat2_val:'99.8%',
    hero_stat2_lbl:'Purity Grade',
    hero_stat3_val:'48h',
    hero_stat3_lbl:'Discreet Shipping',
    hero_card_title:'Research-Grade Quality',
    hero_card_sub:  'Every batch independently tested for purity and potency',

    /* ── Featured (index) ── */
    feat_label: 'Top Sellers',
    feat_title: 'Featured Products',
    feat_sub:   'Our most trusted compounds, loved by the Entre2Fit community.',
    feat_cta:   'View Full Catalog',

    /* ── Trust (index) ── */
    trust_label:  'Why Entre2Fit',
    trust_title:  'Built on Trust & Science',
    trust_sub:    'Every product, every shipment — held to the highest standard.',
    trust1_title: 'Secure Shipping',
    trust1_text:  'All orders are shipped in tamper-proof packaging with end-to-end tracking and full insurance.',
    trust2_title: 'Discreet Packaging',
    trust2_text:  'Privacy is paramount. Every order ships in plain, unmarked packaging with no brand identification.',
    trust3_title: 'Clinical-Grade Quality',
    trust3_text:  'Each batch is independently tested by third-party labs for ≥99% purity before shipping.',

    /* ── Testimonials Preview (index) ── */
    testi_label: 'Customer Stories',
    testi_title: 'What Our Community Says',
    testi_sub:   'Real results from real people who trust Entre2Fit.',
    testi_cta:   'Read All Reviews',

    /* ── CTA Banner (index) ── */
    cta_title: 'Ready to Elevate Your Performance?',
    cta_sub:   'Join thousands of athletes and health-optimizers who trust Entre2Fit.',
    cta_btn:   'Start Shopping',

    /* ── Shop Page ── */
    shop_hero_title: 'Our Product Catalog',
    shop_hero_sub:   'Protocol BALANCE FEEL — everything you need to optimize your body.',
    filter_label:    'Filter:',
    filter_all:      'All Products',
    filter_peptides: 'Kits & Protocols',
    filter_serums:   'Serums',
    filter_stacks:   'Stacks',
    filter_accessories:'Supplements',
    trust_strip_1:   'Secure Checkout',
    trust_strip_2:   'Discreet Packaging',
    trust_strip_3:   '48-Hour Shipping',
    trust_strip_4:   '99.8% Purity',

    /* ── Product Cards ── */
    add_to_cart: 'Add to Cart',
    added:       'Added!',
    out_of_stock:'Out of Stock',

    /* ── Cart Drawer ── */
    cart_title:    'Your Cart',
    cart_empty:    'Your cart is empty',
    cart_subtotal: 'Subtotal',
    cart_checkout: 'Proceed to Checkout',
    cart_shop_btn: 'Shop Now',

    /* ── How to Buy ── */
    htb_hero_title: 'How to Buy',
    htb_hero_sub:   'Simple, secure, and discreet — your complete guide to ordering from Entre2Fit.',
    steps_label:    'The Process',
    steps_title:    'Order in 4 Easy Steps',
    steps_sub:      'From browsing to delivery — we\'ve made it effortless.',
    step1_title:'Browse the Catalog',
    step1_text: 'Explore our full range of clinical-grade peptides and serums, filtered by your needs.',
    step2_title:'Add to Cart',
    step2_text: 'Select your products, choose quantities, and add them to your secure shopping cart.',
    step3_title:'Checkout Securely',
    step3_text: 'Complete your order with our encrypted checkout. Multiple payment options available.',
    step4_title:'Receive Discreetly',
    step4_text: 'Your order ships within 48 hours in plain, unmarked packaging with full tracking.',
    pay_label:  'Payment',
    pay_title:  'Payment Methods',
    pay_sub:    'We accept multiple secure payment options for your convenience.',
    pay1_title: 'Credit / Debit Card',
    pay1_text:  'Visa, Mastercard, and American Express accepted via encrypted gateway.',
    pay2_title: 'PSE',
    pay2_text:  'Secure payments from any bank in Colombia via PSE.',
    pay3_title: 'Bank Transfer',
    pay3_text:  'Direct bank transfers and digital wallets accepted (Nequi/DaviPlata).',
    faq_label:  'FAQs',
    faq_title:  'Frequently Asked Questions',
    faq_sub:    'Everything you need to know before ordering.',
    faq1_q:'Are Entre2Fit products safe?',
    faq1_a:'All our products are independently tested by third-party laboratories for purity (≥99.8%) and safety. They are research-grade compounds intended for qualified individuals.',
    faq2_q:'How long does shipping take?',
    faq2_a:'Orders are processed and shipped within 48 hours. Delivery typically takes 3–7 business days domestically, depending on your location.',
    faq3_q:'Is my order packaged discreetly?',
    faq3_a:'Absolutely. All orders ship in plain, unmarked boxes or envelopes. There is no Entre2Fit branding on the outside, and the sender name is generic.',
    faq4_q:'What is your return policy?',
    faq4_a:'Due to the nature of our products, we do not accept returns on opened items. If your order arrives damaged or incorrect, contact us within 24 hours and we\'ll resolve it immediately.',

    contact_label:  'Get in Touch',
    contact_title:  'Still Have Questions?',
    contact_sub:    'Send us a message and we\'ll respond within 24 hours.',
    contact_name:   'Full Name',
    contact_email:  'Email',
    contact_message:'Message',
    contact_submit: 'Send Message',
    contact_name_ph:   'Your name',
    contact_email_ph:  'you@email.com',
    contact_message_ph:'How can we help?',

    /* ── Testimonials Page ── */
    t_hero_title: "Success Stories",
    t_hero_desc: "Hear from athletes, biohackers, and longevity enthusiasts who trust Entre2Fit for their optimization.",
    t1_text: "\"I had been feeling super bloated for months no matter how healthy I ate. With the Lipo Drean drops and the anti-inflammatory guide, my digestion changed completely in just one week. I lost 4 kg and feel incredibly light.\"",
    t1_title: "Carolina Mendoza — Verified Customer",
    t2_text: "\"The best part is definitely the drops for anxiety! In the afternoons, I was always dying for cookies or chocolates and it ruined my diet. Since using the kit, that anxiety is completely gone. I am so happy with the workout guides.\"",
    t2_title: "Andrea Gómez — Verified Customer",
    t3_text: "\"A very complete and easy-to-follow protocol. You don't starve and the hydration with Himalayan salt gave me so much energy to train. Shipping arrived super fast. Excellent customer service.\"",
    t3_title: "Maria Camila Vargas — Verified Customer",
    t4_text: "\"The detox protocol of the first 24 hours is incredible. I felt an immediate change in the swelling of my abdomen. It is the perfect start to recover your metabolism and feel light again.\"",
    t4_title: "Liliana Castro — Verified Customer",
    t5_text: "\"I loved the anti-inflammatory food guide. The recipes are delicious, easy to prepare, and you really don't starve. The Lipo Drean drops helped me control my sugar cravings entirely.\"",
    t5_title: "Natalia Ramírez — Verified Customer",
    t6_text: "\"I have been on the BALANCE FEEL protocol for three weeks and the results are incredible. I have noticeably lost inches and my energy during the day is totally different. 100% recommended.\"",
    t6_title: "Diana Cardona — Verified Customer",
    t7_text: "\"Absolutely changed my life! The combination of drops and proper hydration got rid of my fluid retention in less than 10 days. I feel lighter and look much more toned.\"",
    t7_title: "Carlos P. — Verified Customer",
    t8_text: "\"Highly recommended! If you are struggling with a slow metabolism and constant bloating, this is the solution. The PDF guides are clear and highly practical.\"",
    t8_title: "Laura W. — Verified Customer",
    t9_text: "\"The customer support answered all my questions about drops dosage within minutes. Delivery to Bogotá took only 24 hours. The kit is worth every single peso!\"",
    t9_title: "Nicolas K. — Verified Customer",

    /* ── Checkout ── */
    checkout_title: "Checkout",
    chk_billing_title: "Billing Details",
    chk_fname: "First name",
    chk_lname: "Last name",
    chk_company: "Company name (optional)",
    chk_country: "Country / Region",
    chk_address: "Street address",
    chk_city: "Town / City",
    chk_state: "State / County",
    chk_zip: "Postcode / ZIP",
    chk_phone: "Phone",
    chk_email: "Email address",
    chk_notes: "Order notes (optional)",
    chk_order_title: "Your order",
    chk_th_product: "Product",
    chk_th_subtotal: "Subtotal",
    chk_subtotal: "Subtotal",
    chk_shipping: "Shipping",
    chk_shipping_calc: "Calculated at next step",
    chk_shipping_note: "(Local delivery usually free)",
    chk_total: "Total",
    chk_pay_crypto: "PSE",
    chk_pay_crypto_desc: "Pay securely with PSE from any bank.",
    chk_pay_bank: "Direct Bank Transfer",
    chk_pay_bank_desc: "Make your payment directly into our bank account or via Nequi/DaviPlata.",
    chk_pay_apps: "P2P Payment Apps",
    chk_pay_apps_desc: "Pay via Zelle, CashApp, or Venmo. Subject to availability.",
    chk_privacy_note: "Your personal data will be used to process your order, support your experience throughout this website, and for other purposes described in our privacy policy.",
    chk_place_order: "Place Order",

    /* ── Footer ── */
    footer_desc:  'Clinical-grade peptide compounds for peak human performance. Science-backed, lab-tested, community-trusted.',
    footer_nav:   'Navigate',
    footer_support:'Support',
    footer_contact:'Contact',
    footer_faqs:  'FAQs',
    footer_privacy:'Privacy Policy',
    footer_terms: 'Terms of Service',
    footer_copy:  '© 2024 Entre2Fit. All rights reserved.',
    no_products:  'No products found in this category.',

    /* ── Product Detail Page (PDP) ── */
    pdp_water_alert_title: '',
    pdp_water_alert_text:  '',
    pdp_water_alert_link:  '',
    pdp_water_alert_end:   '',
    pdp_faq_title:         'Frequently Asked Questions',
    pdp_faq1_q:            'How is shipping done?',
    pdp_faq1_a:            'We ship within Colombia in 1 to 3 business days, depending on the city and logistics availability.',
    pdp_faq2_q:            'Is packaging discreet?',
    pdp_faq2_a:            'Yes, we guarantee completely discreet packaging with no external markings that reveal the contents.',
    pdp_faq3_q:            'Who is the protocol designed for?',
    pdp_faq3_a:            'It is designed for anyone looking to reduce body inflammation, eliminate water retention, control anxiety, and reactivate their metabolism naturally.',
    pdp_faq4_q:            'What are the payment methods?',
    pdp_faq4_a:            'PAYMENT METHODS: BANK TRANSFER, PSE AND CREDIT CARD.',
    pdp_desc_section:      'Product Description',
    pdp_desc_title_suffix: 'in Colombia',
    pdp_features_title:    'Product Features',
    pdp_feat_presentation: 'Presentation:',
    pdp_feat_pres_val:     'Dropper / Complete Kit',
    pdp_feat_use:          'Use:',
    pdp_feat_use_val:      'Daily wellness & anti-inflammation',
    pdp_feat_purity:       'Quality:',
    pdp_feat_purity_val:   '100% natural & safe',
    pdp_feat_shipping:     'Shipping:',
    pdp_feat_shipping_val: 'Fast within Colombia',
    pdp_avail_title:       'Availability & Shipping',
    pdp_avail_text:        'We ship within Colombia in',
    pdp_avail_text2:       '1 to 3 business days',
    pdp_avail_text3:       ', depending on the city and logistics availability.',
    pdp_stock_in:          'In Stock — Ships in 24h',
    pdp_stock_low:         'Low Stock — Only',
    pdp_stock_low2:        'left',
    pdp_stock_out:         'Out of Stock',


  },

  es: {
    /* ── Navigation ── */
    nav_home:        'Inicio',
    nav_shop:        'Tienda',
    nav_calc:        'Calculadora',
    nav_how:         'Cómo Comprar',
    nav_testimonials:'Testimonios',

    /* ── Hero (index) ── */
    hero_label:   'Ciencia de Péptidos de Grado Clínico',
    hero_title:   'Eleva Tu Biología.<br><span>Desbloquea el Máximo Rendimiento.</span>',
    hero_sub:     'Entre2Fit ofrece compuestos de péptidos ultrapuros y rigurosamente probados — formulados para atletas, biohackers y buscadores de longevidad que exigen más.',
    hero_cta1:    'Comprar Ahora',
    hero_cta2:    'Cómo Comprar',
    hero_stat1_val:'500+',
    hero_stat1_lbl:'Clientes Satisfechos',
    hero_stat2_val:'99.8%',
    hero_stat2_lbl:'Grado de Pureza',
    hero_stat3_val:'48h',
    hero_stat3_lbl:'Envío Discreto',
    hero_card_title:'Calidad de Grado Investigación',
    hero_card_sub:  'Cada lote probado independientemente para pureza y potencia',

    /* ── Featured (index) ── */
    feat_label: 'Más Vendidos',
    feat_title: 'Productos Destacados',
    feat_sub:   'Nuestros compuestos más confiables, favoritos de la comunidad Entre2Fit.',
    feat_cta:   'Ver Catálogo Completo',

    /* ── Trust (index) ── */
    trust_label:  'Por Qué Entre2Fit',
    trust_title:  'Construido sobre Confianza y Ciencia',
    trust_sub:    'Cada producto, cada envío — mantenido al más alto estándar.',
    trust1_title: 'Envío Seguro',
    trust1_text:  'Todos los pedidos se envían en empaque a prueba de manipulaciones con rastreo completo y seguro total.',
    trust2_title: 'Empaque Discreto',
    trust2_text:  'La privacidad es primordial. Cada pedido se envía en empaque liso y sin marcas sin identificación de marca.',
    trust3_title: 'Calidad de Grado Clínico',
    trust3_text:  'Cada lote es probado de forma independiente por laboratorios de terceros para ≥99% de pureza antes del envío.',

    /* ── Testimonials Preview (index) ── */
    testi_label: 'Historias de Clientes',
    testi_title: 'Lo Que Dice Nuestra Comunidad',
    testi_sub:   'Resultados reales de personas reales que confían en Entre2Fit.',
    testi_cta:   'Leer Todas las Reseñas',

    /* ── CTA Banner (index) ── */
    cta_title: '¿Listo para Elevar tu Rendimiento?',
    cta_sub:   'Únete a miles de atletas y optimizadores de salud que confían en Entre2Fit.',
    cta_btn:   'Comenzar a Comprar',

    /* ── Shop Page ── */
    shop_hero_title: 'Nuestro Catálogo de Productos',
    shop_hero_sub:   'Protocolo BALANCE FEEL — todo lo que necesitas para optimizar tu cuerpo.',
    filter_label:    'Filtrar:',
    filter_all:      'Todos los Productos',
    filter_peptides: 'Kits y Protocolos',
    filter_serums:   'Sueros',
    filter_stacks:   'Stacks',
    filter_accessories:'Suplementos',
    trust_strip_1:   'Pago Seguro',
    trust_strip_2:   'Empaque Discreto',
    trust_strip_3:   'Envío en 48 Horas',
    trust_strip_4:   '99.8% Pureza',

    /* ── Product Cards ── */
    add_to_cart: 'Agregar al Carrito',
    added:       '¡Agregado!',
    out_of_stock:'Sin Stock',

    /* ── Cart Drawer ── */
    cart_title:    'Tu Carrito',
    cart_empty:    'Tu carrito está vacío',
    cart_subtotal: 'Subtotal',
    cart_checkout: 'Proceder al Pago',
    cart_shop_btn: 'Ir a la Tienda',

    /* ── How to Buy ── */
    htb_hero_title: 'Cómo Comprar',
    htb_hero_sub:   'Simple, seguro y discreto — tu guía completa para ordenar en Entre2Fit.',
    steps_label:    'El Proceso',
    steps_title:    'Ordena en 4 Sencillos Pasos',
    steps_sub:      'Desde la navegación hasta la entrega — lo hemos hecho sin esfuerzo.',
    step1_title:'Explora el Catálogo',
    step1_text: 'Explora nuestra gama completa de péptidos y sueros de grado clínico, filtrados según tus necesidades.',
    step2_title:'Agrega al Carrito',
    step2_text: 'Selecciona tus productos, elige cantidades y agrégalos a tu carrito de compras seguro.',
    step3_title:'Paga de Forma Segura',
    step3_text: 'Completa tu pedido con nuestro pago encriptado. Múltiples opciones de pago disponibles.',
    step4_title:'Recibe Discretamente',
    step4_text: 'Tu pedido se envía dentro de las 48 horas en empaque liso y sin marcas con rastreo completo.',
    pay_label:  'Pago',
    pay_title:  'Métodos de Pago',
    pay_sub:    'Aceptamos múltiples opciones de pago seguras para tu conveniencia.',
    pay1_title: 'Tarjeta de Crédito / Débito',
    pay1_text:  'Visa, Mastercard y American Express aceptadas a través de una pasarela encriptada.',
    pay2_title: 'PSE',
    pay2_text:  'Pagos seguros desde cualquier banco en Colombia a través de PSE.',
    pay3_title: 'Transferencia Bancaria',
    pay3_text:  'Transferencias bancarias directas y billeteras digitales aceptadas (Nequi/DaviPlata).',
    faq_label:  'Preguntas Frecuentes',
    faq_title:  'Preguntas Frecuentes',
    faq_sub:    'Todo lo que necesitas saber antes de ordenar.',
    faq1_q:'¿Son seguros los productos de Entre2Fit?',
    faq1_a:'Todos nuestros productos son probados de forma independiente por laboratorios de terceros para pureza (≥99.8%) y seguridad. Son compuestos de grado investigación destinados a personas calificadas.',
    faq2_q:'¿Cuánto tiempo tarda el envío?',
    faq2_a:'Los pedidos se procesan y envían dentro de las 48 horas. La entrega tarda 3–7 días hábiles a nivel nacional, dependiendo de tu ubicación.',
    faq3_q:'¿Mi pedido se empaca discretamente?',
    faq3_a:'Absolutamente. Todos los pedidos se envían en cajas o sobres lisos y sin marcas. No hay marca de Entre2Fit en el exterior y el nombre del remitente es genérico.',
    faq4_q:'¿Cuál es su política de devoluciones?',
    faq4_a:'Debido a la naturaleza de nuestros productos, no aceptamos devoluciones de artículos abiertos. Si tu pedido llega dañado o incorrecto, contáctanos dentro de las 24 horas y lo resolveremos de inmediato.',

    contact_label:  'Contáctanos',
    contact_title:  '¿Aún Tienes Preguntas?',
    contact_sub:    'Envíanos un mensaje y responderemos en menos de 24 horas.',
    contact_name:   'Nombre Completo',
    contact_email:  'Correo Electrónico',
    contact_message:'Mensaje',
    contact_submit: 'Enviar Mensaje',
    contact_name_ph:   'Tu nombre',
    contact_email_ph:  'tu@correo.com',
    contact_message_ph:'¿En qué podemos ayudarte?',

    /* ── Testimonials Page ── */
    t_hero_title: "Historias de Éxito",
    t_hero_desc: "Escucha a atletas, biohackers y entusiastas de la longevidad que confían en Entre2Fit para su optimización.",
    t1_text: "\"Llevaba meses sintiéndome súper hinchada por más que intentaba comer sano. Con las gotas Lipo Drean y la guía antiinflamatoria, mi digestión cambió del cielo a la tierra en solo una semana. He bajado 4 kilos y me siento súper ligera.\"",
    t1_title: "Carolina Mendoza — Cliente Verificada",
    t2_text: "\"¡Lo mejor son las gotas para la ansiedad! Por las tardes siempre me moría por galletas o chocolates y me arruinaba la dieta. Desde que uso el kit, esa ansiedad desapareció por completo. Estoy feliz con las guías de ejercicios.\"",
    t2_title: "Andrea Gómez — Cliente Verificada",
    t3_text: "\"Un protocolo muy completo y fácil de seguir. No te matas de hambre y la hidratación con la sal rosada me dio mucha energía para entrenar. El envío llegó súper rápido a Medellín. Excelente atención al cliente.\"",
    t3_title: "Maria Camila Vargas — Cliente Verificada",
    t4_text: "\"El protocolo detox de las primeras 24 horas es increíble. Sentí un cambio inmediato en la inflamación de mi abdomen. Es el inicio perfecto para recuperar tu metabolismo y sentirte ligera.\"",
    t4_title: "Liliana Castro — Cliente Verificada",
    t5_text: "\"Me encantó la guía de alimentación antiinflamatoria. Las recetas son deliciosas, fáciles de preparar y realmente no pasas hambre. Las gotas Lipo Drean me ayudaron a controlar los antojos por completo.\"",
    t5_title: "Natalia Ramírez — Cliente Verificada",
    t6_text: "\"Llevo tres semanas con el protocolo BALANCE FEEL y los resultados son increíbles. He bajado medidas notablemente y mi energía durante el día es otra. Lo recomiendo al 100%.\"",
    t6_title: "Diana Cardona — Cliente Verificada",
    t7_text: "\"¡Cambió mi vida por completo! La combinación de las gotas y la hidratación correcta eliminó mi retención de líquidos en menos de 10 días. Me siento ligera y me veo mucho más tonificada.\"",
    t7_title: "Carlos P. — Cliente Verificado",
    t8_text: "\"¡Súper recomendado! Si estás luchando con un metabolismo lento y constante hinchazón, esta es la solución. Las guías en PDF son muy claras y prácticas.\"",
    t8_title: "Laura W. — Cliente Verificada",
    t9_text: "\"La atención al cliente resolvió todas mis dudas sobre la dosis de las gotas en minutos. El envío a Bogotá tardó solo 24 horas. ¡El kit vale cada centavo!\"",
    t9_title: "Nicolas K. — Cliente Verificado",

    /* ── Checkout ── */
    checkout_title: "Finalizar Compra",
    chk_billing_title: "Detalles de Facturación",
    chk_fname: "Nombre",
    chk_lname: "Apellido",
    chk_company: "Nombre de la empresa (opcional)",
    chk_country: "País / Región",
    chk_address: "Dirección",
    chk_city: "Ciudad",
    chk_state: "Estado / Departamento",
    chk_zip: "Código Postal",
    chk_phone: "Teléfono",
    chk_email: "Correo electrónico",
    chk_notes: "Notas del pedido (opcional)",
    chk_order_title: "Tu pedido",
    chk_th_product: "Producto",
    chk_th_subtotal: "Subtotal",
    chk_subtotal: "Subtotal",
    chk_shipping: "Envío",
    chk_shipping_calc: "Calculado en el siguiente paso",
    chk_shipping_note: "(El envío local suele ser gratis)",
    chk_total: "Total",
    chk_pay_crypto: "PSE",
    chk_pay_crypto_desc: "Paga de forma segura desde cualquier cuenta bancaria a través de PSE.",
    chk_pay_bank: "Transferencia Bancaria / Nequi",
    chk_pay_bank_desc: "Realiza tu pago directamente en nuestra cuenta bancaria o billetera digital.",
    chk_pay_apps: "Billeteras Digitales (P2P)",
    chk_pay_apps_desc: "Paga vía Zelle, CashApp, o Nequi/DaviPlata (Colombia). Sujeto a disponibilidad.",
    chk_privacy_note: "Tus datos personales se utilizarán para procesar tu pedido, mejorar tu experiencia y otros propósitos descritos en nuestra política de privacidad.",
    chk_place_order: "Realizar Pedido",

    /* ── Footer ── */
    footer_desc:  'Compuestos de péptidos de grado clínico para el máximo rendimiento humano. Respaldado por ciencia, probado en laboratorio, confiado por la comunidad.',
    footer_nav:   'Navegar',
    footer_support:'Soporte',
    footer_contact:'Contacto',
    footer_faqs:  'Preguntas Frecuentes',
    footer_privacy:'Política de Privacidad',
    footer_terms: 'Términos de Servicio',
    footer_copy:  '© 2024 Entre2Fit. Todos los derechos reservados.',


    /* ── Product Page (PDP) ── */
    pdp_add_cart:    'Añadir al Carrito',
    pdp_buy_now:     'Comprar Ahora',
    pdp_info_title:  'Información del Producto',
    pdp_info_desc:   'Estos productos están destinados estrictamente a fines de investigación y desarrollo de laboratorio. No para consumo humano. Manejar con cuidado.',
    pdp_purity_title:'Pureza y Pruebas',
    pdp_purity_desc: 'Todos nuestros productos son rigurosamente probados por terceros (HPLC/MS) garantizando >99% de pureza. Los informes de lote están disponibles bajo pedido.',
    pdp_ship_title:  'Envíos y Devoluciones',
    pdp_ship_desc:   'Los pedidos se procesan en 24 horas. Empaque discreto garantizado. Debido a la naturaleza de estos productos, todas las ventas son finales.',
    pdp_related:     'Productos Relacionados',
    no_products:     'No se encontraron productos en esta categoría.',

    /* ── Product Detail Page (PDP) ── */
    pdp_water_alert_title: '',
    pdp_water_alert_text:  '',
    pdp_water_alert_link:  '',
    pdp_water_alert_end:   '',
    pdp_faq_title:         'Preguntas Frecuentes',
    pdp_faq1_q:            '¿Cómo se realiza el envío?',
    pdp_faq1_a:            'Realizamos envíos dentro de Colombia en 1 a 3 días hábiles, dependiendo de la ciudad y la disponibilidad logística.',
    pdp_faq2_q:            '¿El embalaje es discreto?',
    pdp_faq2_a:            'Sí, garantizamos un empaque completamente discreto sin marcas externas que revelen el contenido de su interior.',
    pdp_faq3_q:            '¿Para quién está diseñado el protocolo?',
    pdp_faq3_a:            'Está diseñado para personas que buscan desinflamar su cuerpo, eliminar la retención de líquidos, controlar la ansiedad y reactivar su metabolismo de manera 100% natural.',
    pdp_faq4_q:            '¿Cuáles son los métodos de pago?',
    pdp_faq4_a:            'MÉTODOS DE PAGO: TRANSFERENCIA ELECTRÓNICA, PSE Y TARJETA DE CRÉDITO.',
    pdp_desc_section:      'Descripción del producto',
    pdp_desc_title_suffix: 'en Colombia',
    pdp_features_title:    'Características del producto',
    pdp_feat_presentation: 'Presentación:',
    pdp_feat_pres_val:     'Gotero / Kit Completo',
    pdp_feat_use:          'Uso:',
    pdp_feat_use_val:      'Bienestar diario y desinflamación',
    pdp_feat_purity:       'Calidad:',
    pdp_feat_purity_val:   '100% natural y seguro',
    pdp_feat_shipping:     'Envío:',
    pdp_feat_shipping_val: 'Rápido dentro de Colombia',
    pdp_avail_title:       'Disponibilidad y envíos',
    pdp_avail_text:        'Realizamos envíos dentro de Colombia en',
    pdp_avail_text2:       '1 a 3 días hábiles',
    pdp_avail_text3:       ', dependiendo de la ciudad y la disponibilidad logística.',
    pdp_stock_in:          'En Stock — Envío en 24h',
    pdp_stock_low:         'Stock Bajo — Solo',
    pdp_stock_low2:        'disponibles',
    pdp_stock_out:         'Sin Stock',
  }
};

/* ── Apply Translations ──────────────────────────────────── */
// Exponer globalmente para que shop.js y otros scripts puedan leer las claves
window.translations = translations;

const applyLang = (lang) => {
  const t = translations[lang];
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.dataset.i18n;
    if (!t[key]) return;
    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
      el.placeholder = t[key];
    } else if (el.dataset.i18nHtml) {
      el.innerHTML = t[key];
    } else {
      el.textContent = t[key];
    }
  });
  // Update html lang attribute
  document.documentElement.lang = lang === 'es' ? 'es' : 'en';
  // Update toggle button
  const btn = document.getElementById('lang-toggle-btn');
  if (btn) btn.textContent = lang === 'es' ? '🇪🇸 ES' : '🇺🇸 EN';
  // Store preference
  localStorage.setItem(LANG_KEY, lang);
  // Notificar a otros scripts (shop.js, etc.) que el idioma cambió
  document.dispatchEvent(new CustomEvent('entre2fit:langchange', { detail: { lang } }));
};

/* ── Toggle ───────────────────────────────────────────── */
const toggleLang = () => {
  const current = localStorage.getItem(LANG_KEY) || 'es';
  applyLang(current === 'es' ? 'en' : 'es');
};

/* ── Init on Load ─────────────────────────────────────── */
document.addEventListener('DOMContentLoaded', () => {
  const saved = localStorage.getItem(LANG_KEY) || 'es';
  applyLang(saved);
  document.getElementById('lang-toggle-btn')?.addEventListener('click', toggleLang);
});
